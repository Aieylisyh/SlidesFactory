...
> 工具：`tools/python/compress_web_assets.py`（依赖 Pillow）· 抽图：`tools/python/extract_pdf_images.py`
...
# 预览将处理的文件，不写盘
python tools/python/compress_web_assets.py --dry-run

# 自定义阈值（KB）与最长边
python tools/python/compress_web_assets.py --min-kb 50 --max-edge 1600
```
...
```text
contents/*.pdf
    │  python tools/python/extract_pdf_images.py --pdf … --out assets/pdf-extracted
    ▼
...
1. 放入 `assets/<子目录>/`
2. 运行 `python tools/python/compress_web_assets.py`
