...
> 实现：`styles/style-guide/08-segment-arrow-share.css`（`.segment-arrow-*`）· `scripts/chrome/segment-arrow.js` · `scripts/core/deck-slide-host.js` → `initSegmentArrow()`
...
- `index.html` 须已引入 `scripts/chrome/segment-arrow.js`。
