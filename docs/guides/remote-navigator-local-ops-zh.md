...
1. 已安装 **Node.js** 和 **Python 3**（见下方「Node.js 说明」）
2. 项目文件夹 `Project` 已在笔记本本地（含 `remoteNavigator/` 目录）
3. **Windows 防火墙**：首次启动若弹窗，选择「允许访问」（专用网络 / 局域网）

### Node.js 说明

双击 `start-remote-server.bat` 使用的是 **Windows 系统 PATH**，不是 Cursor / VS Code 内置终端。

| 情况 | 说明 |
|------|------|
| 在 Cursor 里能运行 `node -v` | 可能是 IDE 自带 Node，**不等于**系统已安装 |
| 双击 bat 报「未找到 Node.js」 | 需从 [nodejs.org](https://nodejs.org/) 安装 LTS，并勾选 **Add to PATH** |
| 安装后仍报错 | 关闭所有 cmd 窗口，或重启电脑后再试 |

安装完成后，打开 **cmd**（不是 Cursor 终端），输入 `node -v` 应显示版本号，再双击 bat。
...
- 主 deck 分享单页：项目根目录 [`SHARE_GUIDE.md`](../SHARE_GUIDE.md)
