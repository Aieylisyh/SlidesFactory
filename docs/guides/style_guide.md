...
> **文档索引**：[`docs/guides/README.md`](README.md) · [`docs/README.md`](../docs/README.md)
...
- **注入**：`scripts/core/deck-viewport.js` → `injectSlideBrandLogos()` 为每个**叶子** `section`（无 nested 子页的栈父级）自动插入，无需手写 HTML  
...
| 规则 | 实现 |
|------|------|
| 布局 | `scripts/core/deck-nav.js` → `bindDeckNavCluster()` 将 `.slide-number` 移入 `.controls` 内的 `.deck-nav-cluster` flex 容器 |
...
| 运行时 | `scripts/core/deck-nav.js` | `bindDeckNavCluster()` DOM 重组（左箭头 · 页码 · 右箭头） |
...
| 规则 | 实现 |
|------|------|
| Logo | 右上角贴齐画布，`max-width: 180px`（`deck-viewport.js` 自动注入） |
...
| 字号 | 上限 `3.5em`（`--slide-title-font-max`），`scripts/core/deck-viewport.js` → `fitSlidePageTitles()` 按 **70% 宽** 与 **标题区可用高度** 二分取最大字号（下限 `0.72em`），结果写入 `--slide-title-fit-size`（px） |
...
| 层 | 文件 | 职责 |
|----|------|------|
| 样式 / token | `01-tokens-base.css` `:root` + `04a-content-header.css` `.content-slide … h2` | 标题区尺寸、下划线、70% 宽、`--slide-title-fit-size` 回退 |
| 运行时 | `scripts/core/deck-viewport.js` | `injectSlideBrandLogos()` · `fitSlidePageTitle()` / `fitSlidePageTitles()` |
| 触发时机 | `deck-reveal.js` / `deck-slide-host.js` | Reveal 初始化后、`window.resize`（debounce）、`slidechanged` |
...
Reveal 内置 `.progress` 轨道 + 独立模块 `shared/scripts/slide-progress.js`（`SlideProgress.init(Reveal)`，在 `scripts/core/deck-reveal.js` 初始化后调用）。**本 deck 始终启用进度条**（`progress: true`），并**常驻**以下两项视觉：
...
| 启用 | `scripts/core/deck-reveal.js` | `Reveal.initialize({ progress: true })` + `SlideProgress.init(Reveal)` |
...
| `styles/style-guide/04a-content-header.css` | 内容页 H2 带 |
| `styles/style-guide/04b-case-gallery.css` | 案例卡、微信截图、B 站 embed、QHD |
| `styles/style-guide/04c-case-match.css` | 对对碰配对游戏 |
| `styles/style-guide/05a-employment.css` | 就业页、商业现象双卡、岗位棱镜 |
| `styles/style-guide/05b-major-closing.css` | 专业 picker、结束页 |
...
新模块开发顺序：读 [`../config/outline.md`](../config/outline.md)（[`OUTLINE_GUIDE.md`](OUTLINE_GUIDE.md)）→ 抽内容 / 图 → 选 layout 类 → 按 [`transition_guide.md`](transition_guide.md) 设转场 → Speaker notes 补细节。
