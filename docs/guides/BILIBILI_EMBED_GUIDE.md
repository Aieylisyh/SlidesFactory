...
> 实现：`scripts/cases/bilibili-embed.js` · 样式：`styles/style-guide/04b-case-gallery.css`（`.slide-cui-tfe-video`、`.bilibili-embed-*`）· 挂载：`scripts/core/deck-slide-host.js` · 参考页：`#/case-cui-tfe-video`（崔同学 TFE 演示视频）
...
| 文件 | 职责 |
|------|------|
| `scripts/cases/bilibili-embed.js` | 翻页时 `mount` / `unmount` iframe；可选拉取播放量 |
| `scripts/core/deck-slide-host.js` | `initBilibiliEmbed()`、`teardownBilibiliEmbed()`；`slidechanged` 时卸载上一页 |
| `styles/style-guide/04b-case-gallery.css` | `.slide-cui-tfe-video`、`.bilibili-embed-frame` 等全屏布局 |
| `index.html` | `<script src="scripts/cases/bilibili-embed.js?v=…">` 须在 `deck-slide-host.js` **之前** |
...
4. **结构**：新视频页先更新 `config/outline.md`，再改 `index.html`
...
- [ ] `config/outline.md` 增加页数 / 小节说明
- [ ] 复制 HTML 骨架，改标题、`bvid`、外链、兜底播放量文案
- [ ] 确认 `data-bilibili-frame` 为空容器
- [ ] 需要新版式时扩展 `04b-case-gallery.css` + `style_guide_extended.md`
