...
| 行为 | 说明 |
|------|------|
| 打开 | 自动定位到 `config/share-pages.json` 中登记的 slide |
...
完整 slug 列表见 [`../../config/share-pages.json`](../../config/share-pages.json)（与每页 `id` 一致）。

## 新增可分享页

1. 给目标 leaf `<section>` 加稳定 **`id`**（建议与 slug 相同，kebab-case）
2. 在 **`config/share-pages.json`** 的 `pages` 中登记 `"slug": { "slideId": "…", "title": "…" }`
...
- `scripts/chrome/share-lock.js` — 解析参数、改 Reveal 配置、锁定
- `scripts/main.js` — 启动前 `ShareLock.init()`（`scripts/chrome/share-lock.js`），条件跳过 deck 导航模块
