...
`outline.md` 是单个 deck 的**结构唯一来源**：章节顺序、页数预算、Reveal `id`、版式类名、脚本依赖与是否冻结。AI 与人工改 `index.html` 前都应先读它。
...
- 页数：写区间时用 `9–12`，明确预算
- 抽图：写完整 CLI，含 `--pdf` / `--out` / `--page`；抽图后追加 `python scripts/compress_web_assets.py`
