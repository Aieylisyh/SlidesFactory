...
> 本演示使用 **Reveal.js 内置转场 = `none`**，页内进入动画由 `scripts/slide-transitions.js` 统一分配。  
...
| 文件 | 职责 |
|------|------|
| `scripts/slide-transitions.js` | 分配逻辑 + Web Animations API 实现 |
| `styles/style-guide/06-transitions-salary.css` — `/* Custom slide transition FX */` | overlay、clip-path、3D 等辅助类 |
| `scripts/main.js` | `transition: 'none'`，ready 后 `SlideTransitionFX.bind(Reveal)` |
