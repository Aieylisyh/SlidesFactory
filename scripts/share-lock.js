 ...
 * Registry: share-pages.json (slug → slideId).
 */
(function (global) {
    'use strict';

    var REGISTRY_URL = (global.ShareLockConfig && global.ShareLockConfig.registryUrl) || 'share-pages.json';
    ...
    function init() {
        ...
        return fetch(REGISTRY_URL)
            .then(function (res) {
                if (!res.ok) throw new Error('share-pages.json load failed');
                return res.json();
            })
