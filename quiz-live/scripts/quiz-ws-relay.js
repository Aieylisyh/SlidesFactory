...
function broadcastStreak(room, participant, streak) {
    broadcast(room, {
        type: 'streak_broadcast',
        name: participant.name,
        streak: streak,
        message: formatStreakMessage(participant.name, streak)
    });
}
...
function handleSelfAnswer(room, client, msg) {
    ...
    if (msg.correct) {
        ...
        if (STREAK_TIERS.indexOf(p.streak) !== -1) {
            broadcastStreak(room, p, p.streak);
        }
    } else {
        p.streak = 0;
    }
